...
public class Fractal {
  ...
  private void drawKochCurve(Point2D p1, Point2D p2, int level) {
    ...
    Turtle object = new Turtle(p1, p2);
    double calculate = p1.distance(p2) / 3;
    Point2D p3;
    Point2D p4;
    Point2D p5;
    object.move(calculate);
    p3 = object.getPosition();   
    object.turnLeft(60);
    object.move(calculate);
    p4 = object.getPosition();
    object.turnRight(120);
    object.move(calculate);
    p5 = object.getPosition();
  ...
  private void drawTree(Point2D p1, Point2D p2, int level) {
    if (level == 0) { // base case
      ...
      return;
        ...
        // TODO
    Turtle turtle1 = new Turtle(p1, p2);
    Turtle turtle2 = new Turtle(p1, p2);
    double calculate = p1.distance(p2);
    Point2D p3, p4, p5;
    turtle1.move(calculate/ 3);
    turtle2.move(calculate/ 3);
    p3 = turtle1.getPosition();
    turtle1.turnLeft(60);
    turtle1.move(2 * (calculate / 3));
    p4 = turtle1.getPosition();
    turtle2.turnRight(15);
    turtle2.move(2 * (calculate / 3));
    p5 = turtle2.getPosition();   
    drawLine(p1, p3);
  ...
  private void drawSierpinskiCarpet(Point2D p, double a, int level) {
    if (level == 0) { // base case
      ...
      Point2D p2, p3, p4, p5, p6, p7, p8;
      double calculate = (a / 3);
      Point2D location = new Point2D.Double(p.getX() + calculate, p.getY());
      Turtle turtle = new Turtle(p, location);
      turtle.move(calculate);
      p2 = turtle.getPosition();
      turtle.move(calculate);
      ...
      drawSierpinskiCarpet(p, calculate, level - 1);
      drawSierpinskiCarpet(p2, calculate, level - 1);
      drawSierpinskiCarpet(p3, calculate, level - 1);
      drawSierpinskiCarpet(p4, calculate, level - 1);
      drawSierpinskiCarpet(p5, calculate, level - 1);
      drawSierpinskiCarpet(p6, calculate, level - 1);
      drawSierpinskiCarpet(p7, calculate, level - 1);
      drawSierpinskiCarpet(p8, calculate, level - 1);
    }
        // TODO
  }
